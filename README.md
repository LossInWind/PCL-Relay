...
网关默认仅监听回环地址。配置远程监听时，必须同时显式设置
`--admin-cidrs`（服务环境变量为 `PCL_RELAY_ADMIN_CIDRS`）；否则安装和启动会拒绝继续。
个人可信 Tailnet 可使用 `100.64.0.0/10,fd7a:115c:a1e0::/48`，不要使用公网通配 CIDR。
该权限同时影响门户 CONNECT 与管理接口。恢复或升级后的验收必须分别检查
`/healthz`、`/v1/models`、`/admin/portal.pac` 和经代理访问门户的 HTTPS 请求；
健康检查通过不能代替门户可用性，也不能代替模型生成成功。
